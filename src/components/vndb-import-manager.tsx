...
export function VNDBImportManager() {
  ...
  async function handleImport() {
    ...
    try {
      ...
      const { ok, data, error } = await apiFetchSafe<{ data?: { message?: string; results?: ImportResult[] } }>("/api/admin/vndb/import", {
        method: "POST",
        body: { vndbIds: ids },
      })
  ...
  return (
    <div className="rounded-xl bg-card p-4 sm:p-6 ring-1 ring-border">
      ...
      <Textarea
        ...
        rows={4}
        className="resize-none px-4 py-3 text-[15px] md:text-[15px] leading-6"
      />
