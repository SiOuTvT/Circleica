...
export function ResourceTab({
  ...
  // 资源动态：官方动态（GameLog，有就一起参与排序）∪ 由已加载资源生成的事件。
  // 不新增表、不新增接口、不写库 —— 每条资源就在这里产出一行「新增资源「名字」」。
  const activityItems = useMemo<ActivityItemData[]>(() => {
    const generated: ActivityItemData[] = resources.map((r) => ({
      id: `resource-created-${r.id}`,
      kind: "resource_created",
      title: `「${r.resourceName || "未命名资源"}」`,
      createdAt: r.createdAt,
    }))
    return [...gameActivities, ...generated]
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
      .slice(0, 8)
  }, [gameActivities, resources])

  // 右栏槽位内容交给页面统一渲染（整页只有一个右栏，宽度与 x 恒定）
  useEffect(() => {
    onRailChange?.({ activities: activityItems, stats: statRows })
  }, [activityItems, statRows, onRailChange])
