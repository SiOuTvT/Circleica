      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {games.map(game => (
            <div
              key={game.id}
              className="rounded-2xl bg-card ring-1 ring-border p-4 sm:p-5 transition-all hover:ring-primary/30"
            >
              {/* 游戏信息 */}
              <Link
              ...
              >
                {game.coverImage ? (
                  <Image
                    ...
                    width={64}
                    height={80}
                    className="h-16 w-12 sm:h-20 sm:w-14 rounded-lg object-cover shrink-0"
                    unoptimized
                  />
                ) : (
                  <div className="h-16 w-12 sm:h-20 sm:w-14 rounded-lg bg-muted flex items-center justify-center shrink-0">
              ...
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
                {game.creators.map(c => (
                  <Link
                    key={`${c.id}-${c.role}`}
                    href={`/creators/${c.id}`}
                    className="flex flex-col items-center gap-1.5 rounded-xl p-2.5 transition-all hover:bg-secondary"
                  >
                    {c.avatar ? (
                      <Image
                        src={c.avatar}
                    ...
                    <div className="text-center min-w-0 w-full">
                      ...
                      <span className={cn(
                        "inline-block mt-0.5 rounded-full px-1.5 py-0.5 text-[10px] font-medium",
