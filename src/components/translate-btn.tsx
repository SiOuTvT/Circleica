...
export function TranslateBtn({ text, onTranslated }: { text: string; onTranslated: (translated: string) => void }) {
  ...
  return (
    <button
      ...
      className="flex items-center gap-1.5 rounded-lg bg-secondary/80 px-3 py-1.5 text-xs font-medium text-foreground ring-1 ring-border transition duration-150 ease-in-out hover:bg-accent hover:text-foreground disabled:opacity-50"
      title="将英文描述翻译为中文"
    >
      {loading ? (
