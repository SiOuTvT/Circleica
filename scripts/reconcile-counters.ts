 ...
 * 严禁任何写操作。所有 SQL 均为本脚本静态 SELECT，无外部输入。
 */
...
const checks: Array<{ name: string; sql: string }> = [
  {
    ...
    sql: `
      SELECT g.id, g."favoriteCount" AS stored, COALESCE(c.cnt,0)::int AS actual,
             (g."favoriteCount" - COALESCE(c.cnt,0))::int AS diff
      FROM "Game" g
      LEFT JOIN (SELECT "gameId", COUNT(*) cnt FROM "Favorite" GROUP BY "gameId") c
        ON c."gameId" = g.id
      WHERE g."favoriteCount" <> COALESCE(c.cnt,0)
      ORDER BY ABS(g."favoriteCount" - COALESCE(c.cnt,0)) DESC
      LIMIT 50`,
  },
  {
    name: "Game.downloadCount vs ResourceDownloadLog",
    sql: `
      SELECT g.id, g."downloadCount" AS stored, COALESCE(c.cnt,0)::int AS actual,
             (g."downloadCount" - COALESCE(c.cnt,0))::int AS diff
      FROM "Game" g
      LEFT JOIN (SELECT "gameId", COUNT(*) cnt FROM "ResourceDownloadLog" GROUP BY "gameId") c
        ON c."gameId" = g.id
      WHERE g."downloadCount" <> COALESCE(c.cnt,0)
      ORDER BY ABS(g."downloadCount" - COALESCE(c.cnt,0)) DESC
      LIMIT 50`,
  },
  {
    name: "Comment.likeCount vs CommentLike",
    sql: `
      SELECT cm.id, cm."likeCount" AS stored, COALESCE(c.cnt,0)::int AS actual,
             (cm."likeCount" - COALESCE(c.cnt,0))::int AS diff
      FROM "Comment" cm
      LEFT JOIN (SELECT "commentId", COUNT(*) cnt FROM "CommentLike" GROUP BY "commentId") c
        ON c."commentId" = cm.id
      WHERE cm."likeCount" <> COALESCE(c.cnt,0)
      ORDER BY ABS(cm."likeCount" - COALESCE(c.cnt,0)) DESC
      LIMIT 50`,
  },
  {
    name: "ForumPost.likeCount vs ForumPostLike",
    sql: `
      SELECT p.id, p."likeCount" AS stored, COALESCE(c.cnt,0)::int AS actual,
             (p."likeCount" - COALESCE(c.cnt,0))::int AS diff
      FROM "ForumPost" p
      LEFT JOIN (SELECT "postId", COUNT(*) cnt FROM "ForumPostLike" GROUP BY "postId") c
        ON c."postId" = p.id
      WHERE p."likeCount" <> COALESCE(c.cnt,0)
      ORDER BY ABS(p."likeCount" - COALESCE(c.cnt,0)) DESC
      LIMIT 50`,
  },
  {
    name: "ForumComment.likeCount vs ForumCommentLike",
    sql: `
      SELECT fc.id, fc."likeCount" AS stored, COALESCE(c.cnt,0)::int AS actual,
             (fc."likeCount" - COALESCE(c.cnt,0))::int AS diff
      FROM "ForumComment" fc
      LEFT JOIN (SELECT "commentId", COUNT(*) cnt FROM "ForumCommentLike" GROUP BY "commentId") c
        ON c."commentId" = fc.id
      WHERE fc."likeCount" <> COALESCE(c.cnt,0)
      ORDER BY ABS(fc."likeCount" - COALESCE(c.cnt,0)) DESC
      LIMIT 50`,
  },
]
...
async function main(): Promise<void> {
  ...
  for (const c of checks) {
    try {
      const data = (await prisma.$queryRawUnsafe(c.sql)) as Array<{
        id: string
        stored: number
        actual: number
        diff: number
