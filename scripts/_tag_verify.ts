...
const extractH1 = (html: string): string | null => {
  const m = html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/)
  return m ? m[1].replace(/<[^>]+>/g, "").trim() : null
}
const countCards = (html: string): number => (html.match(/href="\/games\//g) || []).length
const isNotFound = (html: string): boolean => html.includes("页面不存在")
...
async function main() {
  ...
  let ok = 0
  const fail: Array<{ slug: string; name: string; status?: number; notFound?: boolean; hasName?: boolean; error?: string }> = []
  for (const t of tags) {
